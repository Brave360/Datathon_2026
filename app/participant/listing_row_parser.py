...
def _derive_features(
    ...
    feature_values = {
        ...
        "private_laundry": _merge_optional_bools(
            _parse_bool(row.get("washing_machine")),
            _feature_list_flag(
                feature_keys,
                list_present=feature_list_present,
...
def prepare_listing_row(row: dict[str, str]) -> tuple[Any, ...]:
    ...
    return (
        ...
        _clean_text(row.get("scrape_source", "").strip("'\"")),
        title,
        description,
        street,
        city,
        ...
        _parse_int(row.get("number_of_bedrooms")),
        _parse_int(row.get("number_of_bathrooms")),
        _parse_int(row.get("year_built")),
        _parse_int(row.get("last_renovation")),
        offer_type,
